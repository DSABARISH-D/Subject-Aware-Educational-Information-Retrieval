...
def ingest_file(
    ...
    if existing:
        action = "UPDATE"
        db.query(Chunk).filter(Chunk.document_id == existing.id).delete(synchronize_session=False)
        document = existing
    ...
    result = {"document": document, "action": action, "pages": len(pages), "chunks": len(page_chunks), "embeddings": len(embeddings), "inserted": len(page_chunks)}
    if logger:
